                ...
                >
                    ...
                    <div className="flex items-center justify-between px-6 py-4 bg-slate-50/50 border-b border-slate-100">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <span className="text-lg font-bold text-slate-800">审计任务执行中</span>
                                <div className="text-xs text-slate-500 font-medium mt-0.5">任务 ID: {taskId}</div>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <Button
                                ...
                                className="w-8 h-8 rounded-full hover:bg-slate-100 text-slate-400"
                            >
                    ...
                    <div className="flex h-[calc(100%-80px)]">
                        ...
                        <div className="w-64 p-6 border-r border-slate-100 bg-slate-50 flex flex-col gap-6">
                            <div className="space-y-2">
                                <div className="text-xs font-bold text-slate-500 uppercase tracking-wider">Type</div>
                                <div className="flex items-center gap-3 bg-white p-3 rounded-lg border border-slate-200 shadow-sm">
                                    {taskType === 'repository'
                                        ? <div className="p-1.5 bg-blue-50 rounded-md"><Cpu className="w-4 h-4 text-primary" /></div>
                                        : <div className="p-1.5 bg-amber-50 rounded-md"><HardDrive className="w-4 h-4 text-amber-500" /></div>}
                                    <span className="text-sm font-bold text-slate-700 uppercase tracking-wide">{taskType === 'repository' ? 'Repository' : 'Zip Upload'}</span>
                                </div>
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                {isCancelled ? (
                                    <Badge variant="outline" className="w-full justify-center py-2 bg-amber-50 text-amber-600 border-amber-200 font-bold">CANCELLED</Badge>
                                ) : isCompleted ? (
                                    <Badge variant="outline" className="w-full justify-center py-2 bg-emerald-50 text-emerald-600 border-emerald-200 font-bold">COMPLETED</Badge>
                                ) : isFailed ? (
                                    <Badge variant="outline" className="w-full justify-center py-2 bg-rose-50 text-rose-600 border-rose-200 font-bold">FAILED</Badge>
                                ) : (
                                    <Badge variant="outline" className="w-full justify-center py-2 bg-blue-50 text-primary border-blue-200 font-bold animate-pulse">RUNNING</Badge>
                                )}
                            </div>
                            ...
                            <div className="p-4 bg-white border border-slate-200 rounded-lg shadow-sm">
                                ...
                                <p className="text-xs text-slate-500 leading-relaxed">
                                    系统正在分析您的代码。此过程可能需要几分钟，具体取决于项目规模。请勿关闭此窗口。
                                </p>
                            </div>
                        </div>

                        {/* Terminal Screen */}
                        <div className="flex-1 flex flex-col bg-slate-900 border-l border-slate-200">
                            {/* Terminal Header */}
                            <div className="h-8 bg-slate-950 flex items-center px-4 gap-2 border-b border-slate-800">
                                ...
                                <span className="ml-2 text-[10px] uppercase font-mono text-slate-500">terminal_output.log</span>
                            </div>

                            {/* Terminal Output */}
                            <div className="flex-1 p-6 overflow-y-auto font-mono text-sm custom-scrollbar bg-slate-900 text-slate-300">
                                <div className="space-y-1 pb-10">
                                    {logs.map((log) => (
                                        <div key={log.id} className="flex items-start gap-3 hover:bg-white/5 px-2 py-0.5 transition-colors rounded">
                                            <span className="text-slate-500 text-xs flex-shrink-0 w-20 font-mono select-none">
                                                {log.timestamp}
                                            </span>
                                            <span className={`${getLogColor(log.type)} flex-1 font-mono text-sm`}
                                                style={{ textShadow: log.type === 'success' ? '0 0 10px rgba(52, 211, 153, 0.2)' : 'none' }}>
                                                {log.message}
                                            </span>
                                        </div>
                                    ))}
                                    ...
                                    {!isCompleted && !isFailed && !isCancelled && (
                                        <div className="flex items-center gap-3 mt-4 px-2">
                                            <span className="text-slate-500 text-xs w-20 font-mono select-none">{currentTime}</span>
                                            <span className="text-slate-400 animate-pulse font-bold">_</span>
                                        </div>
                                    )}
                                    <div ref={logsEndRef} />
                                </div>
                            ...
                            {/* Bottom Controls */}
                            <div className="h-16 px-6 border-t border-slate-800 bg-slate-950 flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-3">
                                    {!isCompleted && !isFailed && !isCancelled && (
                                        <Button
                                            ...
                                            className="h-8 bg-transparent border-amber-500/50 text-amber-500 hover:bg-amber-500/10 hover:border-amber-500 hover:text-amber-400 font-mono uppercase text-xs"
                                        >
                                    ...
                                    {isFailed && (
                                        <Button
                                            ...
                                            className="h-8 bg-slate-800 border-slate-700 text-slate-300 hover:bg-slate-700 font-mono uppercase text-xs"
                                        >
                                    ...
                                    {(isCompleted || isFailed || isCancelled) && (
                                        <Button
                                            size="sm"
                                            onClick={() => onOpenChange(false)}
                                            className="h-8 bg-emerald-600 hover:bg-emerald-500 text-white font-mono uppercase text-xs border-none"
                                        >
